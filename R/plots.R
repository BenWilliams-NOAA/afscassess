...
plot_biomass <- function(year, model) {
  ...
  read.csv(here::here(year, model, "processed", "mceval.csv"))  %>%
    ...
    tidytable::mutate.(year = as.numeric(gsub("tot_biom_", "", name)),
                       name = "Total biomass") %>%
    tidytable::bind_rows.( read.csv(here::here(year, model, "processed", "mceval.csv")) %>%
                                              tisytable::select(paste0("spawn_biom_", yrs)) %>%
                                              tisytable::mutate.(group = 1) %>%
                                              tisytable::pivot_longer.(-group) %>%
                                              tisytable::mutate.(year = as.numeric(gsub("spawn_biom_", "", name)),
...
#'
#' @examples
plot_phase <- function(year, folder, model_name){
...
#'
#' @examples
plot_rec <- function(year, folder, rec_age){
  ...
  read.csv(here::here(year, folder, "processed", "ages_yrs.csv"))$yrs -> yrs
  read.csv(here::here(year, folder, "processed", "bio_rec_f.csv")) %>%
  dplyr::select(recruits) %>%
  dplyr::bind_cols(read.csv(here::here(year, folder, "processed", "mceval.csv")) %>%
...
ggplot2::ggsave(here::here(year, folder, "figs", "recruits.png"), width = 6.5, height = 5.5, units = "in", dpi = 200)
}
...
plot_rec_ssb <- function(year, folder, rec_age){
...
ggplot2::ggsave(here::here(year, folder, "figs", "recr-ssb.png"), width = 6.5, height = 5.5, units = "in", dpi = 200)
}
...
plot_selex <- function(year, folder){
  ...
  read.csv(here::here(year, folder, "processed", "selex.csv")) %>%
    dplyr::select(age, Fishery = fish, Survey = srv1, Maturity = maturity) -> dat
  ...
  dat %>%
    ...
    ggplot2::scale_x_continuous(name = "Year",
                                breaks = afscassess::tickr(dat, age, 10,
                                                           start = 0)$breaks,
                                labels = afscassess::tickr(dat, age, 10,
                                                           start = 0)$labels) +
    ggplot2::theme(legend.justification=c(1,0), legend.position=c(0.9,0.2))

  ggplot2::ggsave(here::here(year, folder, "figs", "selex.png"), width = 6.5, height = 5.5, units = "in", dpi = 200)
}
...
plot_survey <- function(year, folder){

  if (!dir.exists(here::here(year, folder, "processed"))){
    stop("must run 'process_results' before creating figures")
  }

  read.csv(here::here(year, folder, "processed", "survey.csv")) %>%
...
plot_swath <- function(year, folder){
  ...
  read.csv(here::here(year, folder, "processed", "ages_yrs.csv"))$yrs -> yrs
  read.csv(here::here(year, folder, "processed", "b35_b40_yld.csv")) -> bby

  read.csv(here::here(year, folder, "processed", "mceval.csv")) %>%
  ...
  dat %>%
    ...
    ggplot2::geom_ribbon(data = dplyr::filter(dat, year<yr),
                         ggplot2::aes(ymin = min, ymax = max, fill = fill), alpha = 0.07) +
    ...
    ggplot2::geom_hline(yintercept = c(bby$B40/1000, bby$B35/1000),
                        lty = c(3, 1)) +
    scico::scale_fill_scico_d(palette = 'grayC', begin = 1)

  ggplot2::ggsave(here::here(year, folder, "figs", "swath.png"), width = 6.5, height = 5.5, units = "in", dpi = 200)
}
...
plot_params <- function(year, model, model_name, pars = c("q_srv1", "ABC", "nattymort", "tot_biom", "F40","spawn_biom")) {
  if (!dir.exists(here::here(year, model, "processed"))){
    stop("must run 'process_results()' before creating figures")
  }
